...
### Fixed
- **Зависание при запуске**: `platform.uname()` в Python 3.13 вызывает `cmd /c ver` через subprocess, что зависает на некоторых Windows-конфигурациях. Заполняем кэш через `sys.getwindowsversion()` до любых импортов
- **Зависание при транскрибации**: `imageio_ffmpeg` тоже вызывал `platform.system()` → тот же `cmd /c ver`, блокируя обнаружение FFmpeg
- **30-минутное зависание fact-checker**: Anthropic API-вызов с `web_search` мог висеть бесконечно без timeout. Добавлен `timeout=60s` на каждый запрос
- **Нет обратной связи при старте обработки**: GUI показывал "Готов к работе" пока шло извлечение аудио. Теперь статус обновляется сразу

### Changed
- **Ранний экспорт**: файлы транскрипции (MD, JSON, SRT, VTT, TXT) экспортируются ДО AI-анализа — результаты на диске даже если анализ зависнет
- **Circuit breaker в fact-checker**: после 2 подряд неудач Anthropic API → автопереключение на DuckDuckGo для всех оставшихся claims
- **Timeout в link resolver**: добавлен `timeout=60s` на Anthropic API-вызовы
...
### Fixed
- Защита от rate limit (429) Anthropic API при факт-чекинге: пауза 3 сек между запросами
- Retry с экспоненциальным ожиданием (30/60 сек) при rate limit вместо мгновенного провала
- DDG fallback на уровне отдельного claim — если один запрос упал, остальные продолжают через DDG
- Отключены встроенные retry SDK (слишком быстрые для rate limit)

### Changed
- Link resolver: батчинг по 5 сущностей с паузами между батчами для защиты от rate limit
- Link resolver: retry с ожиданием при 429 перед fallback на DDG
